...
const cases = [
    ...
    { id: 9, caseNumber: '#9', title: 'Beekeeper\'s Secret', description: 'A beekeeper uses his hives to pass secret messages for a spy ring.', image: 'https://images.pexels.com/photos/1192609/pexels-photo-1192609.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1', rotation: 1 },
    { id: 10, caseNumber: '#10', title: 'The Final Curtain', description: 'The entire conspiracy is set to be revealed on opening night.', image: 'https://images.pexels.com/photos/33129/popcorn-movie-party-entertainment.jpg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1', rotation: 3 },
];

const DESKTOP_TRACK_WIDTH_PERCENT = 250;

const desktopPositions = {
    1: { top: '15%', left: '5%' }, 2: { top: '50%', left: '18%' }, 3: { top: '10%', left: '31%' }, 4: { top: '55%', left: '44%' }, 5: { top: '20%', left: '57%' },
    6: { top: '15%', left: '70%' }, 7: { top: '50%', left: '83%' }, 8: { top: '10%', left: '96%' }, 9: { top: '55%', left: '109%' }, 10: { top: '20%', left: '122%' }
};
...
const CaseClue = ({ caseData, isDesktop, setPinRef }) => {
    ...
    return (
        <motion.div
            className={isDesktop ? "absolute z-10 cursor-pointer group w-[250px]" : "relative w-[90%] max-w-[320px] z-10"}
            style={isDesktop ? { top: position.top, left: position.left, transform: `translateX(-50%)` } : {}}
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.3 }}
            transition={{ duration: 0.6 }}
        >
            <div ref={(el) => setPinRef(caseData.id, el)} className="absolute -top-2.5 left-1/2 -translate-x-1/2 w-5 h-5 z-20"></div>
            <div className="transition-transform duration-300 ease-in-out group-hover:scale-105" style={{ transform: `rotate(${caseData.rotation}deg)` }}>
                <div className="relative">
                    ...
                    <div className="flex flex-col items-center">
                        <div className="bg-white p-2.5 shadow-lg border border-gray-200 w-full"><img src={caseData.image} alt={caseData.title} className="w-full h-auto object-cover" /></div>
                        <div className="bg-yellow-200 p-2 text-center text-4xl font-bold text-gray-800 shadow-md w-[80px] -mt-5 z-10" style={{ transform: `rotate(${(Math.random() - 0.5) * 8}deg)` }}>{caseData.caseNumber}</div>
                        <div className="bg-gray-200 p-3 shadow-sm text-gray-700 leading-relaxed w-[90%] text-center -mt-2" style={{ transform: `rotate(${(Math.random() - 0.5) * 6}deg)` }}>
                            <h3 className="text-lg font-bold mb-1 text-red-700">{caseData.title}</h3>
...
const Events = () => {
    ...
    const calculatePath = useCallback(() => {
        if (!isDesktop || Object.keys(pinRefs.current).length < cases.length) return;
        ...
        const pathData = cases.slice(1).map((_, index) => {
            ...
            const p1x = prevRect.left - containerRect.left + scrollContainer.scrollLeft + prevRect.width / 2;
            const p1y = prevRect.top - containerRect.top + prevRect.height / 2;
            const p2x = currentRect.left - containerRect.left + scrollContainer.scrollLeft + currentRect.width / 2;
            const p2y = currentRect.top - containerRect.top + currentRect.height / 2;
    ...
    return (
        <div className="bg-[#121212] font-['Special_Elite',_cursive]">
            <Header />
            <main className="w-full min-h-screen pt-28 pb-16 px-4 flex flex-col items-center justify-center overflow-x-hidden">
                ...
                >
                    <div className="relative w-full h-full" style={isDesktop ? { width: `${DESKTOP_TRACK_WIDTH_PERCENT}%` } : {}}>
                        <div className="lg:hidden relative w-full h-auto flex flex-col items-center gap-y-32">
                           ...
                           {cases.map((caseData) => (
                               <CaseClue key={caseData.id} caseData={caseData} isDesktop={false} setPinRef={setPinRef} />
                           ))}
                        </div>
                        <div className="hidden lg:block absolute top-0 left-0 w-full h-full">
                           ...
                           {cases.map((caseData) => (
                               <CaseClue key={caseData.id} caseData={caseData} isDesktop={true} setPinRef={setPinRef} />
                           ))}
                        </div>
                    </div>
                </div>
